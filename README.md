...
| フリーズ | **バッファは足りているのに絵が止まった**回数と累積時間 | 下記参照 |
...
**キャッシュから返ったセグメントは集計から外している。** 外さないと余裕度が桁で狂う。実測では VOD を巡回監視する構成で、6.3MB のセグメントが 18ms で「ダウンロード」され、余裕度が 200〜700 になる状態が全体の半分を占めた。回線を測れていないので、そのままでは判断材料にならない。

判定は2段構えになっている。

- Resource Timing の `transferSize` が 0 かつ本文サイズが取れていればキャッシュと確定する
- 判定できない場合は、**同じセグメントURLの2回目以降の取得**を除外する（VOD の巡回では2回目以降はまず確実にキャッシュから返る。LIVE ではURLが再出現しないので影響しない）

2段目が要るのは、`Timing-Allow-Origin` を返さない配信では `transferSize` が読めず、キャッシュかどうかを原理的に判定できないため。書き出しの `from_cache` 列は3値で、**空欄は「判定できなかった」であって「キャッシュではない」ではない**。`segment_repeat` 列と併せて読むこと。

### stall とフリーズは別物

`stall` はバッファが尽きて再生が止まること。`waiting` イベントで捕まえられる。

**バッファが潤沢なまま絵だけ止まることがある。** リバッファではないので `waiting` は発火せず、`stall` では検出できない。実測では、バッファ20秒を保ったまま4秒間デコードフレームが1枚も進まず、再生時計だけが進み続ける状態を観測した。視聴者にはフリーズそのものだが、従来は完全に素通りしていた。

これを `フリーズ` として別に数える。判定の条件は3つ。

- デコードフレーム数が1枚も増えていない
- **再生時計は進んでいる**（これが無いと、一時停止から再生に戻った直後を必ず誤検知する）
- **タブが表示されている**（`document.visibilityState`。裏タブや画面消灯ではブラウザが描画を止めるので、条件に入れないと誤検知だらけになる）

書き出しの `visible` 列で、事後にタブの可視状態を確認できる。
...
| 列 | 注意 |
|---|---|
| `frame` | **どのプレーヤーの行か**。1ページに複数のプレーヤーが iframe で並ぶ構成では、これが無いと行を区別できない |
...
- **フリーズの検出はサンプリング間隔（既定1秒）より短い停止を捉えられない**。デコードフレーム数の差分で見ているため。また、再生時計が進んでいることを条件にしているので、時計ごと止まる停止は stall 側で数える
